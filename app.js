...
const DOMAIN = 'sandboxbbbab63499234a40abb930bf7f1effe2.mailgun.org';
const api_key_mailgun = "2e39bda1075836f9afb6a33045c147c9-f135b0f1-e43345e7";
const mg = mailgun({ apiKey: api_key_mailgun, domain: DOMAIN });
...
function sendTweet(targetUser, enabledLocationServices, lat, lon) {
	...
	if (!enabledLocationServices) {
		tweetStatus = "Hey @" + targetUser + ", we can't see where you are!" +
									...
									" location private or you can't use a mobile device, check out our website: <link>"
	} else {
...
function emailResults(hospData, user_email) {

	var str_hosp_info = "Thank you for donating PPE to hospitals near you. </br> As you requested, here is the contact information for the hospitals near you <br><br>";
	...
	// 	to: 'sam.ewenczyk@gmail.com', // use variable
	// 	subject: 'results test',
	// 	text: str_hosp_info
	// };

	const data = {
		from: "PPE Donations Service <postmaster@sandboxbbbab63499234a40abb930bf7f1effe2.mailgun.org>",
		to: user_email,
...
app.get("/queryhospitals/", function(req, res) {
  ...
  res.send(JSON.stringify({
    ...
    hospitals: hospData
  	...
  	emailResults(hospData, user_email);
}); 
